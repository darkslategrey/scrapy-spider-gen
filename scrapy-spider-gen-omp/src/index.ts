...
export default function (pi: ExtensionAPI) {
	// omp injects both the typebox builders (`pi.typebox.Type`) and zod
	// (`pi.zod`) through the ExtensionAPI. `Type` mirrors
	// `@sinclair/typebox`'s namespace; zod is used for the level enum since
	// @oh-my-pi/pi-ai dropped the legacy `StringEnum` helper.
	const { Type } = pi.typebox;
	// pi.zod is the zod module itself — alias it rather than destructure so
	// TS sees the namespace type and accepts `.enum(...)`, `.string()`, etc.
	const z = pi.zod;
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			level: z
				.enum(["light", "normal", "aggressive"] as const)
				.describe(
					"Cleaning level: " +
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			// ── 1. Fetch HTML ──────────────────────────────────────────────
			onUpdate?.({
				content: [{ type: "text", text: `⏳ Fetching ${args.source}…` }],
			});

			let raw: string;
			try {
				raw = await fetchHtml(args.source, ctx.cwd, signal);
			} catch (err) {
				throw new Error(
			...
			onUpdate?.({
				content: [
					{ type: "text", text: `🧹 Cleaning HTML (mode: ${args.level})…` },
				],
			});

			const result = cleanHtml(raw, args.level);
			...
			let templateContent = "";
			if (args.template) {
				const absPath = resolve(ctx.cwd, args.template);
				try {
